...
${BUILD_DIR}is-online: $(OBJ)
	$(CC) -o ${BUILD_DIR}is-online is_online.cc $^ `pkg-config --cflags --libs gtkmm-3.0 libcurl sqlite3`

${BUILD_DIR}run_tests: $(OBJ) $(TEST_OBJ)
	$(CC) -o ${BUILD_DIR}run_tests run_tests.cc /usr/lib/x86_64-linux-gnu/libgtest.a $^ `pkg-config --cflags --libs gtkmm-3.0 libcurl sqlite3 gtest`
